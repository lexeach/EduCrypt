  ...
  if (window.ethereum) {
    ...
    try {
      ...
      if (accounts.length > 0) {
        ...
        await checkUserStatus(); // Check user registration status after connecting
      } else {
        showBuyCourseButton(); // Show "Buy Course" button by default if not connected
      }
    } catch (error) {
...
async function registerUser() {
  ...
  let loaderOverlay;
  try {
    let _referrerID = 1; // Default value, can be modified as needed
    let _coreferrerID = 1; // Default value, can be modified as needed
...
async function checkUserStatus() {
  if (!userAccount) {
    return; // If no user account, exit the function silently
  }

  try {
    const contract = new web3.eth.Contract(contractABI, contractAddress);
    const isRegistered = await contract.methods.users(userAccount).call();
    console.log("Is registerd: ", isRegistered[0]);
    if (isRegistered[0]) {
      showOpenCourseButton(); // Show "Open Course" button if user is registered
    } else {
      showBuyCourseButton(); // Show "Buy Course" button if user is not registered
    }
  } catch (error) {
...
function showOpenCourseButton() {
  ...
  document
    ...
    .addEventListener("click", function () {
      window.location.href = "courseEnglish.html"; // Change to the actual course page URL
    });
}
